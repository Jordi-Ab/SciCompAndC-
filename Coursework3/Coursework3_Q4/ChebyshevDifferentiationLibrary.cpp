...
Matrix ChebyshevDifferentiationLibrary::LaplacianDifferentiationMatrix(const Vector& x_grid,
                                                                       const Vector& y_grid){
    Matrix Dx_2 = SecondOrderDifferentiationMatrix(x_grid);
    Matrix Dy_2 = SecondOrderDifferentiationMatrix(y_grid);
    Matrix Ix = eye(x_grid.GetSize());
    Matrix Iy = eye(y_grid.GetSize());
    return Dx_2.KroneckerProduct(Iy) + Ix.KroneckerProduct(Dy_2);
}

Matrix ChebyshevDifferentiationLibrary::eye(int n){
    Matrix identity(n, n);
    for(int r=0; r<n; r++){
        for(int c=0; c<n; c++){
            if(r==c) identity(r+1,c+1) = 1;
            else identity(r+1,c+1) = 0;
        }
    }
    return identity;
}
