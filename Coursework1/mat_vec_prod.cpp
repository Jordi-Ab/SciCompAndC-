/*
Function: dot
Usage: double dot_prod = dot(one_vec, another_vec);
-------------------
...
Receives:
	."matrix" -> A matrix with n_rows and any number of columns,
	."vector" -> A vector with n_rows and 1 column.
...
double* matrixTimesVector(double **matrix, double *vector, 
							int mat_rows, int mat_cols, int vec_rows){
	double* result;
